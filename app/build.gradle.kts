...
tasks.register("generateBp") {
    ...
    val kapt = project.configurations["kapt"]
    ...
    // Update host plugins
    kapt.resolvedConfiguration.resolvedArtifacts.sortedBy {
        moduleString(it.moduleVersion.id)
    }.distinctBy {
        moduleString(it.moduleVersion.id)
    }.forEach {
        ...
        // Get file path
        val dirPath = "${libsBase}/host/${modulePath(id)}"
        val filePath = "${dirPath}/${it.file.name}"
        ...
        File("$libsBase/Android.bp").let { file ->
            ...
            file.appendText(
                """
                        ...
                        java_import_host {
                            name: "${moduleName(id)}-host",
                            jars: ["host/${modulePath(id)}/${it.file.name}"],
                        }
